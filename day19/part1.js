...
function explore(blueprint, state, resource, time, banned) {
    //console.log('explore', blueprint, state, resource, time, banned);
...
for(var b = 0; b < blueprints.length; b++) {
    ...
    explore(b, [1, 0, 0, 0], [0, 0, 0, 0], 24, [0, 0, 0, 0]);
}

console.log(mostGeodesPerBlueprint.map((s, i) => s*(i+1)).reduce((a, c) => a + c, 0));